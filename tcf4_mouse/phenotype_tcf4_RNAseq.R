...
###################################
## move files to globus directory #
###################################

p = "/dcl02/lieber/ajaffe/NatNeuro_TCF4_Data/FASTQ"
r = c(pd$R1,pd$R2)
r = r[file.exists(r)]
parallel::mclapply(r, file.copy, p,mc.cores=8) # run once

## fix paths
pd$R1 = sapply(strsplit(pd$R1, "/"), function(x) x[length(x)])
pd$R2 = sapply(strsplit(pd$R2, "/"), function(x) x[length(x)])
pd$bamFile = NULL
write.csv(pd, 'tables/maher_philpot_PTHS_mouse_rnaseq_pheno_relativePaths.csv', quote = F, row.names = F)
