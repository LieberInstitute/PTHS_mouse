### Sweatt mouse differential expression analysis 
# qsub -V -l mf=10G,h_vmem=15G,h_stack=256M -cwd -b y R CMD BATCH analyze_sweatt.R
...
load('/dcl01/lieber/ajaffe/Brady/sweatt/rpkmCounts_sweatt_OCT20_n16.rda')
pd = cbind(dat$pd,pd)
...
plot(pcaGene$x[,2]~pd$totalAssignedGene,bg=as.numeric(pd$Genotype), pch=21,
     xlab = "Gene Assignment Rate", ylab = "PC2", main = "PC2 v. Gene Assignment Rate", cex.lab = 1.25)
legend("bottomright", legend = levels(pd$Genotype),pch =21, pt.bg = 1:3,cex= 1.25)

plot(pcaGene$x[,2]~pd$mitoRate,bg=as.numeric(pd$Genotype), pch=21,
     xlab = "Mito Map Rate", ylab = "PC2", main = "PC2 v. MitoRate", cex.lab = 1.25)
legend("bottomright", legend = levels(pd$Genotype),pch =21, pt.bg = 1:3,cex= 1.25)
...
sum(sigGene$fdr<0.05) #76 genes FDR significant
...
WriteXLS(list(Gene = sigGene,Exon= sigExon, Junction = sigJxn, phenotype = pd),
         ExcelFileName = 'tables/sweatt_DE_table.xls')
save(svaGene, svaExon, svaJxn, file="rdas/sweatt_sva_objects.rda")
save(pd, outGene, outExon, outJxn, file="rdas/sweatt_DE_objects.rda")
