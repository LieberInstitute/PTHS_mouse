...
plot(outExon2$pos,outExon2$logFC,xlab = 'Exon #',ylab = 'Log2 Fold-change',
     ylim = ylim,main = 'Mouse Tcf4 Transcript 2 Fold-change',cex.lab = 1.25,bg = outExon2$col,pch = 21)
...
################################
# plot last coded coded exon chr18:69682683-69682823, e672980
tcf4Exon = 2^cleaningY(yExon[rownames(outExon),],modExon, P = 4)-1
pd$ExonExpr = tcf4Exon['e672980',]
pd$fdr = outExon['e672980',c('padj')]

pdf("tcf4_mouse/plots/fig1_tcf4_last_exon.pdf",height = 3.5, width = 4)
ggplot(data=pd) +
  ...
  ylab("Adjusted Counts | SVs")+xlab("") + theme(axis.text.x=element_text(angle=45,hjust = 1)) +
