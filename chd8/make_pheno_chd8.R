...
manifest = with(pd,paste(FASTQ.gz, 0, Sample_s,sep = '\t'))

cat(manifest, file = paste0(dir,'/samples.manifest'),sep = '\n')

##########################
# run the RNA seq pipeline
# /dcl01/lieber/ajaffe/Emily/RNAseq-pipeline/sh/rnaseq-run-all.sh --experiment chd8 --prefix feb6 --reference mm10 --stranded true --ercc FALSE --cores 8 --large FALSE --fullcov FALSE